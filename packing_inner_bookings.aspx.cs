...
public partial class packing_inner_bookings : System.Web.UI.Page
{
    ...
    {
        if (RadioButtonList1.SelectedItem.Text == "Select Status")
        {


            Response.Write("<script>alert('Choose packing status')</script>");
        ...
        else
        {
            ...
            if (bs == "Booked")
            {
                ...
                Response.Write("<script>alert('Production status not updated')</script>");


            }
            else if (bs == "Packing finished")
            {


                Response.Write("<script>alert('Packing status already updated')</script>");
            ...
            else
            {
                ...
                MySqlCommand cmda = new MySqlCommand("select zone from pay where pcode='" + Session["pid"].ToString() + "'", cona);
                ...
                if (RadioButtonList1.SelectedItem.Text == "Packing finished")
                {
                    ...
                    MyMailMessage.Subject = "Hai, this is from Feast. Currrent status of your food order";
                    MyMailMessage1.Subject = "New food order for delivery";



                    MyMailMessage.Body = "Current status of your food order with order code " + pc + " is " + RadioButtonList1.SelectedItem.Text + " .There is no refunding for paid amount.";
                    MyMailMessage1.Body = "New food order for delivery  from " + zone + " .Login and check for details";
                ...
                Response.Write("<script>alert('Packing status updated')</script>");
