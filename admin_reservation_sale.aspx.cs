...
public partial class admin_zone_sale : System.Web.UI.Page
{
    ...
    private void BindGrid()
    {
        ...
        MySqlDataAdapter adapt = new MySqlDataAdapter("SELECT reservation.payid,reservation.hours,reservation.rdate,reservation.no_of_seats,reservation.type,reservation.time,reservation.date,reservation.bstatus,reservation.amount,reservation.name,reservation.phone,reservation.status,reservation.mode FROM reservation ", con);
        con.Open();
        adapt.Fill(dt);
        con.Close();
